...
export const AddressTable = ({ companiesDto, rows, setRows }) => {
  ...
  const getAllProvincias = async () => {
    try {
      ...
      const nombresProvincias = provinciasResponse.data.provincias.map(
        (prov) => prov.nombre
      );
  ...
  const getAllLocalidades = async (prov) => {
    try {
      const localidadesProv = await axios.get(
        `https://apis.datos.gob.ar/georef/api/localidades?provincia=${
          prov ? prov : "seleccione"
        }&campos=nombre&max=1000`
      );
      const uniqueLocalidades = Array.from(
        new Set(localidadesProv.data.localidades.map((local) => local.nombre))
      );
